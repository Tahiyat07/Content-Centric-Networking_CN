...
//            << interestMsg->getDataName() << endl;
    ...
    if (csEntry != NULL) {
        ...
        contentObjMsg->setHeaderSize(PHOENIX_CONTENT_OBJECT_MSG_HEADER_SIZE);
        ...
        contentObjMsg->setByteLength(PHOENIX_CONTENT_OBJECT_MSG_HEADER_SIZE + csEntry->payloadSize);
    ...
    while (iteratorFaceEntry != fibEntry->forwardedFaces.end()) {
        ...
        if (faceEntry->faceID != arrivalFaceEntry->faceID) {
            ...
            newInterestMsg->setHeaderSize(PHOENIX_INTEREST_MSG_HEADER_SIZE);
            newInterestMsg->setPayloadSize(interestMsg->getPayloadSize());
            newInterestMsg->setHopsTravelled(interestMsg->getHopsTravelled() + 1);
            newInterestMsg->setByteLength(PHOENIX_INTEREST_MSG_HEADER_SIZE + 0);
...
//            << contentObjMsg->getDataName() << endl;
    ...
    for (int i = 0; i < pitEntry->arrivalInfoList.size(); i++) {
        ...
        newContentObjMsg->setHeaderSize(PHOENIX_CONTENT_OBJECT_MSG_HEADER_SIZE);
        ...
        newContentObjMsg->setByteLength(PHOENIX_CONTENT_OBJECT_MSG_HEADER_SIZE + contentObjMsg->getPayloadSize());
